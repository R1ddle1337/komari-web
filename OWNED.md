...
- 安装脚本来自 `R1ddle1337/komari-agent` 的 `1.5.17` tag。
- 默认初次安装传入 `--install-version 1.5.17`，Agent 仍可从其自管仓库自动更新。
- 保留最新 UI 的指定版本与 snapshot 选项；显式选择的版本仍只从自管 Agent 仓库下载。
- Docker 使用 `ghcr.io/r1ddle1337/komari-agent:1.5.17`。
...
设置卡片按字段保存，避免旧快照覆盖其他修改。节点列表同时刷新合并，并取消卸载后的请求；Ping 任务列表保留正确的加载与失败状态。
诊断页增加主控/前端版本、Go 运行时、概览请求耗时和刷新时间，摘要读取有超时边界。未注册通知渠道提供插件配置入口，不自动启用通知。
前端依赖按已确认告警更新，DOMPurify 固定安全补丁，npm audit（含开发依赖）为 0；保留 Monaco 与路由现有功能。

## 性能优化：1.5.8 主控配套前端

历史读取自动请求可选紧凑格式（points_v1），在传输层还原时间、数值、计数和标签；旧服务器
普通格式继续兼容。管理首页直接使用 PingStats 返回的 P95，删除仅为重复计算排行而拉取的
完整延迟历史；流量曲线、CPU/内存排序和按需迷你图保持原有数据与统计口径。

首装 Agent 固定为自有 1.5.17。测试覆盖紧凑格式往返、空值/权重、旧格式兼容以及真实图表渲染。
